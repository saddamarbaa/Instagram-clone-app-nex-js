...
const DynamicPost: React.FunctionComponent<PostType> = ({
	postContent,
	postImage,
	postDate,
	userName,
		...
		<div className='mt-[1.5rem] bg-white shadow-sm   max-w[616px] overflow-hidden py-4 border  rounded-[5px] '>
			<div className=' px-4 flex items-center space-x-4 justify-between  w-full'>
				<div className=' flex items-center  space-x-4 '>
					{userImage && <User image={userImage} width={35} height={35} />}
					<p className=' font-semibold text-[14px]'>{userName}</p>
				</div>
				<DotsHorizontalIcon className='h-6' />
			</div>

			<div className='mt-[30px] px-4  min-h-[50px] w-full text-[15px] text-gray-500'>
				{postContent}
			</div>

			{postImage && (
				<div className='relative w-full h-[300px] mt-5 lg:h-[600px]'>
					<Image
						src={postImage}
						alt='Post Image'
						layout='fill'
