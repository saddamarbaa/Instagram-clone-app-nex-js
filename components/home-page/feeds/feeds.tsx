...
import { fakeUsersForStoryes } from "../../../lib/api-util";
	...
	return (
		<div className='flex-auto  lg:mr-8 relative overflow-x-hidden'>
			...
			<div className='bg-white shadow-sm flex space-x-4 max-w-[616px] overflow-hidden py-6 border  rounded-[5px]  pl-4'>
				...
				<div>
					<User image='/images/sadam.jpg' width={60} height={60} />
					<p className='text-[12px] pl-2 mt-1'>Saddam Arbaa </p>
				</div>

				{fakeUsersForStoryes &&
					fakeUsersForStoryes?.map((user) => {
						return (
							<div key={user?.id}>
								<User image={user?.avatar} width={60} height={60} />
